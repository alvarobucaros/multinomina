    ...
    <div class="card-body">
      ...
      <p class="card-text">
        ...
        <table class="table table-sm table-hover">
            ...
            <tbody>  
                <tr>
                    <td>{{ $dependencias->dep_nombe}}</td>                                
                </tr>
            </tbody>
        </table>
        <hr>

        <form action="{{ route('dependencias/dependencias.destroy', $dependencias->id) }}" method="POST">
            @csrf
            @method('DELETE')
              <a href="{{route('dependencias')}} " class="btn btn-sm btn-info"> 
